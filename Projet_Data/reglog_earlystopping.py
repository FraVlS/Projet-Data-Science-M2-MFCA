...
corr_mask = np.abs(corr) > 0.6  # Create a boolean mask for values above 0.6 in absolute
corr_highlighted = corr.copy()  # Make a copy of the correlation matrix
corr_highlighted[~corr_mask] = np.nan  # Set values below 0.6 in absolute to NaN
...
model.save('final_logistic_regression_model.h5')
