...
def compute_aic(model, X, y):
    ...
    # Évite les problèmes numériques (log(0))
    eps = 1e-9
    y_proba = np.clip(y_proba, eps, 1 - eps)
    
    # Log-vraisemblance (formule du modèle logistique)
    log_likelihood = np.sum(y * np.log(y_proba) + (1 - y) * np.log(1 - y_proba))
    
    # Nombre de paramètres = coefficients + intercept
    n_params = model.named_steps['model'].coef_.shape[1] + 1
...
import MLstatkit as MLs
def compute_CI(y_true, y_proba):
    CI = MLs.Bootstrapping(y_true, y_proba, metric_str='ROC_AUC')
    return CI 
